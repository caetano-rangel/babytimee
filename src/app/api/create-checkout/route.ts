...
export async function POST(req: NextRequest) {
  try {
    ...
    const sexo           = formData.get('sexo')           as string ?? 'menina';
    ...
    const slug = `${nomeBebe.toLowerCase().replace(/\s+/g, '-')}-${nanoid(10)}`;

    // Upload das fotos para o Supabase Storage
    const fotoUrls: string[] = [];
    for (const foto of fotos) {
      const fileName = `${slug}-${nanoid(10)}.${foto.type.split('/')[1]}`;
      const { error: uploadError } = await supabase.storage
        .from('uploads').upload(fileName, foto, { upsert: false });
